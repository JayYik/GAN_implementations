...
class DCGAN_D(nn.Module):
    ...
    def __init__(self, hw, channels):
        ...
        self.main_module = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True))
            # outptut of main module --> State (4*dim, hw//8, hw//8)

        self.output = nn.Sequential(
            nn.Conv2d(in_channels=4*self.dim, out_channels=1, kernel_size=hw//8, stride=1, padding=0),
            # Output (1, 1, 1)
            nn.Sigmoid())
...
class DCGAN_G(nn.Module):
    ...
    def __init__(self, hw, z_dim,channels):
        ...
        self.main_module = nn.Sequential(
            # Z latent vector
            nn.ConvTranspose2d(in_channels=z_dim, out_channels=4*self.dim, kernel_size=hw//8, stride=1, padding=0),
